...
module.exports = class TimeTracker {
    ...
    this.askUrl()
      ...
      .then(() => {
        ...
        const saveNext = () => {
          ...
          const validUrl = this.validateUrl(rc.url);
          if (!validUrl) {
          ...
          httpClient.post(validUrl + '/tracking/save', formData)
            .then((response) => {
  ...
  info () {
    this.askUrl()
      ...
      .then(() => {
        ...
        const baseUrl = this.validateUrl(this.rc.url);
        if (!baseUrl) {
          console.error('Invalid URL in configuration'.red);
  ...
  askUrl() {
    ...
    return Util.ask('TimeTracker URL' + (rc.url ? ' <' + rc.url + '>' : '') + ': ').then((url) => {
      ...
      return httpClient.get(validUrl)
        .then((response) => {
    ...
    return new Promise((resolve) => {
      Util.ask('User name' + (this.rc.user ? ' <' + this.rc.user + '>' : '') + ': ').then((user) => {
        ...
        Util.ask('Password: ', true).then((password) => {
          const validUrl = this.validateUrl(this.rc.url);
          if (!validUrl) {
            console.error('Invalid URL in configuration'.red);
            return this.login().then(resolve);
          }
          const formData = new URLSearchParams({ username: validUsername, password, loginCookie: 'on' });
          httpClient.post(validUrl + '/login', formData)
            .then((response) => {
  ...
  askEntity(type) {
    ...
    return new Promise((resolve) => {
      httpClient.get(validUrl + '/' + paths[type])
        .then((response) => {
