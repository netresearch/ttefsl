...
module.exports = class AbstractLogger {
  ...
  setEntries(rawEntries) {
    ...
    rawEntries.forEach((entry) => {
      const fromDay = [entry.from.getUTCFullYear(), entry.from.getUTCMonth() + 1, entry.from.getUTCDate()].join('-');
      const toDay = [entry.to.getUTCFullYear(), entry.to.getUTCMonth() + 1, entry.to.getUTCDate()].join('-');
      if (fromDay !== toDay) {
